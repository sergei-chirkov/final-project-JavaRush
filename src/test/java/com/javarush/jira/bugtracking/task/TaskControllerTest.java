...
class TaskControllerTest extends AbstractControllerTest {
    ...
    @Test
    @WithUserDetails(value = ADMIN_MAIL)
    void checkGetTimeInWorkAndTesting() throws Exception{
        MvcResult mvcResult = perform(MockMvcRequestBuilders.get(ACTIVITIES_REST_URL_SLASH + 3))
                .andExpect(status().isOk())
                .andDo(print())
                .andReturn();

        String content = mvcResult.getResponse().getContentAsString();
        assertEquals("Task was in progress 23:59:03 Task was in testing 1 days 00:02:56",content);
    }
