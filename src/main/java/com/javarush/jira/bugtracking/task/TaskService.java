...
public class TaskService {
    ...
    public String getTimeTaskInWork(long taskId){
        List<Activity> activities = activityHandler.getRepository().findStatusInProgress(taskId);
        LocalDateTime inProgressTaskTime = null;
        LocalDateTime readyForReviewTaskTime= null;
        for(Activity activity:activities){
            if(activity.getStatusCode().equals("in_progress")){
                inProgressTaskTime = activity.getUpdated();
            }
            if(activity.getStatusCode().equals("ready_for_review")){
                readyForReviewTaskTime = activity.getUpdated();
            }
        }

        String betweenTime = getBetweenTime(inProgressTaskTime,readyForReviewTaskTime);

        if(betweenTime != null){
            return "Task was in progress " + betweenTime;
        }

        if(inProgressTaskTime == null){
            return "Task has status IN_PROGRESS";
        }

        return "Task is absent";
    }
    public String getTimeTaskInTesting(long taskId){
        List<Activity> activities = activityHandler.getRepository().findStatusIsDone(taskId);
        LocalDateTime isDoneTaskTime = null;
        LocalDateTime readyForReviewTaskTime= null;
        for(Activity activity:activities){
            if(activity.getStatusCode().equals("done")){
                isDoneTaskTime = activity.getUpdated();
            }
            if(activity.getStatusCode().equals("ready_for_review")){
                readyForReviewTaskTime = activity.getUpdated();
            }
        ...
        String betweenTime = getBetweenTime(readyForReviewTaskTime,isDoneTaskTime);

        if(betweenTime != null){
            return "Task was in testing " + betweenTime;
        }

        if(isDoneTaskTime == null){
            return "Task has status READY_FOR_REVIEW";
        }

        return "Task id" + taskId + "isn't being testing";
    }


    private String getBetweenTime(LocalDateTime first, LocalDateTime second){
        if(first!=null & second!=null){

            Duration between = Duration.between(first, second);
            LocalTime localTime = LocalTime.of(between.toHoursPart(),between.toMinutesPart(), between.toSecondsPart());

            if(between.toDays() == 0){
                return localTime.toString();
            }
            return between.toDays() + " days " + localTime.toString();
        }
        return null;
    }
